...
#ALGORITHM_CONFIGS = ['m545','rovio','vins_mono','vins_mono_lc']
...
OVERALL_BOXPLOT_DISTANCES = [2.0, 3.0, 6.0, 8.0]
...
def compute_odometry_error_per_dataset(dataset_trajectories_dict,
    ...
    for dataset_idx, dataset_nm in enumerate(dataset_names):
        ...
        distances = [np.around(x*traj_length,decimals=1)
...
if __name__ == '__main__':
    ...
    print("\n>>> Compute trajectory configuration table ...")
    data_table = {}
    data_table['values'] = []
    data_table['rows'] =  datasets
    data_table['cols'] =  ["Length","Duration","Avg. lin. vel.","Std. lin. vel.","Max. lin. vel.","Avg. ang. vel.","Std. ang. vel.","Note"]
    for traj in config_trajectories_list[0]:
        cur_data = []
        print("> Processing {0}".format(traj.uid))
        traj.compute_velocity()
        cur_data.append("{:3.1f}".format(traj.accum_distances[-1]))
        cur_data.append("{:3.0f}".format(traj.t_gt[-1]-traj.t_gt[1]))
        cur_data.append("{:3.2f}".format(traj.velocity['lin_vel']['mean']))
        cur_data.append("{:3.2f}".format(traj.velocity['lin_vel']['std']))
        cur_data.append("{:3.2f}".format(traj.velocity['lin_vel']['max']))
        cur_data.append("{:3.2f}".format(traj.velocity['ang_vel']['mean']))
        cur_data.append("{:3.2f}".format(traj.velocity['ang_vel']['std']))     
        cur_data.append(" ")
        data_table['values'].append(cur_data)
    print("data_table['values']",data_table['values'])
    print("<<< ...computing  trajectory configuration table done.\n")
